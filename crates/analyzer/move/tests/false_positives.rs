//! False-positive and true-positive regression corpus for the move analyzer.
//!
//! `tests/corpus/good/*.move` are programs correct by construction and must
//! produce **zero findings**. `tests/corpus/bad/*.move` carry a real bug and
...
fn corpus(sub: &str) -> Vec<(String, String)> {
    ...
    let mut files: Vec<PathBuf> = fs::read_dir(&dir)
        ...
        .collect();
    files.sort();
    assert!(!files.is_empty(), "no .move files under {}", dir.display());
    files
        .into_iter()
        .map(|p| {
